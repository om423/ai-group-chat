...
export function startDocAnalyst() {
  on("file.uploaded", async ({ fileId, roomId }: { fileId: string; roomId: string }) => {
    ...
    if (!sf) {
      console.warn(`DocAnalyst: File ${fileId} not found in FileStore`);
      return;
    }
    ...
    try {
      ...
      const analysis = normalizeAnalysis(fileId, roomId, sf, extract, json);
      ...
      const text = makeAgentPost(analysis);
      await execPost(
        { type: "Agent", id: "doc-analyst", name: "DocAnalystAgent", orgId: "org-1" },
        () => postAsAgentImpl(roomId, text)
      );
      ...
      console.log(`DocAnalyst: Successfully analyzed ${sf.filename} (${fileId})`);
      ...
      console.error(`DocAnalyst: Failed to analyze ${sf.filename} (${fileId}):`, e);
      
      // Post a failure message
      try {
        await execPost(
          { type: "Agent", id: "doc-analyst", name: "DocAnalystAgent", orgId: "org-1" },
          () => postAsAgentImpl(roomId, `❌ Failed to analyze ${sf.filename}: ${e.message}`)
        );
      } catch (postError) {
        console.error("DocAnalyst: Failed to post error message:", postError);
      }
    }
  });
}
